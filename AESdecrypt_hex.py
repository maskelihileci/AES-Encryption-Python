...
end = 32  # set ending pointer for the part to decrypt of the plaintext
...
# Set up the segment message loop parameters
for y in range(1, int(loopmsg)):  # loop to encrypt all segments of the message
    ...
    outputhex += myhexstring  # Collect the hex output
    outputbv = BitVector(hexstring=myhexstring)
    asciioutput += outputbv.get_bitvector_in_ascii()

asciioutput = asciioutput.replace('\x00', '')
FILEOUT.write("Decrypted message (ASCII):\n")
